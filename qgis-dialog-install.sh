...
if [ -d "$app_path_1" ]; then
    ...
    if [ "$dialogResults" = 0 ]; then
        ...
        "$dialogPath" \
        --title "$dialogTitleA" \
        --message "\n\nAfter QGIS is installed, open Finder and click Applications, then find QGIS.  \n\nHold down the Command (⌘) key and right-click (click with two fingers on a Mac trackpad), then click “Open”. \n\n![Gatekeeper](https://resources.jwainfrastructure.com/img/qgis-launch-sonoma.png)" \
        --icon "$dialogIcon" \
        --bannerimage "$dialogBannerImage" \
        --bannerheight 85 \
        --bannertitle "$dialogBannerTitleA" \
